...
import ca.uwinnipeg.proximitydroid.v2.RotatedBitmap;
...
implements OnSharedPreferenceChangeListener {
  ...
  // The probe functions
  protected Map<String, ImageFunc> mProbeFuncs = new HashMap<String, ImageFunc>();
  ...
  public void onCreate() {
    ...
    // Load probe funcs
    Map<String, List<ImageFunc>> features = getProbeFuncs();
    for (String catStr : features.keySet()) {
      for (ImageFunc func : features.get(catStr)) {
   ...
   * @return
   */
  public Map<String, List<ImageFunc>> getProbeFuncs() {
    Map<String, List<ImageFunc>> features = new HashMap<String, List<ImageFunc>>();
    // load all the standard probe funcs
    features.put("Colour", new ArrayList<ImageFunc>());
    List<ImageFunc> colourFuncs = features.get("Colour");
    colourFuncs.add(new AlphaFunc());
    colourFuncs.add(new RedFunc());
    colourFuncs.add(new GreenFunc());
    colourFuncs.add(new BlueFunc());
    
    features.put("Edge", new ArrayList<ImageFunc>());
    List<ImageFunc> edgeFuncs = features.get("Edge");
    edgeFuncs.add(new DifferentialExcitationFunc());
  ...
  public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
    ...
    if (mProbeFuncs.keySet().contains(key)) {
      ImageFunc func = mProbeFuncs.get(key);
